...
interface ProductCardProps {
  product: Product;
  hrefBase?: string;
  // Employee/company portals add via their own cart flows — hide the public quick-add there.
  showAddToCart?: boolean;
}

export default function ProductCard({ product, hrefBase = '/products', showAddToCart = true }: ProductCardProps) {
  const imageSrc = product.images[0] ?? null;
  const hasDiscount = product.originalMinPrice > product.minPrice && product.minPrice > 0;

  return (
    <div className="group font-display relative flex flex-col rounded-[20px] border border-white/85 bg-white/60 p-3.5 shadow-[0_12px_34px_rgba(34,36,90,.1)] backdrop-blur-[14px] transition-shadow hover:shadow-[0_16px_44px_rgba(34,36,90,.18)]">
      ...
      {showAddToCart && (
        <div className="mt-auto pt-3">
          <AddToCartMini slug={product.slug} />
        </div>
      )}
