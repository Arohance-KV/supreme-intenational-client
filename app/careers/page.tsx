...
const depts = ['All', 'Engineering', 'Design', 'Operations', 'Sales'];

const jobs = [
  { role: 'Senior Frontend Engineer', dept: 'Engineering', loc: 'Bangalore · Hybrid', type: 'Full-time' },
  { role: 'Backend Engineer (Node)', dept: 'Engineering', loc: 'Bangalore · Hybrid', type: 'Full-time' },
  { role: 'Product Designer', dept: 'Design', loc: 'Bangalore · Hybrid', type: 'Full-time' },
  { role: 'Catalogue Operations Lead', dept: 'Operations', loc: 'Bangalore · On-site', type: 'Full-time' },
  { role: 'Production Coordinator', dept: 'Operations', loc: 'Bangalore · On-site', type: 'Full-time' },
  { role: 'Enterprise Account Manager', dept: 'Sales', loc: 'Remote · India', type: 'Full-time' },
];
      ...
      <div className="relative z-[1] mx-auto max-w-[1180px] px-6">
        ...
        <section className="pb-4 pt-2">
          ...
          <div className="flex flex-col gap-3">
            {list.map((j) => (
              <div key={j.role} className="flex flex-wrap items-center gap-4 rounded-2xl border border-white/80 bg-white/55 px-[22px] py-[18px] shadow-[0_8px_24px_rgba(34,36,90,.06)] backdrop-blur-[14px]">
                <div className="flex-1">
                  <div className="mb-1 text-base font-bold">{j.role}</div>
                  <div className="text-xs text-slate">{j.dept} · {j.loc}</div>
                </div>
                <span className="font-jbmono rounded-full bg-[rgba(23,155,142,.12)] px-[11px] py-[5px] text-[11px] font-semibold text-accent">{j.type}</span>
                <Link href="/contact" className="rounded-[11px] border border-[rgba(42,43,106,.12)] bg-[rgba(42,43,106,.07)] px-[18px] py-2.5 text-[13px] font-semibold text-indigo no-underline">Apply →</Link>
              </div>
            ))}
          </div>
        </section>
